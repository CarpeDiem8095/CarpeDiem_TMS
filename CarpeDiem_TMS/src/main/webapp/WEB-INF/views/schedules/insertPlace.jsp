...
<%-- 							<input type="hidden" value="${placeDto.place_seq}"> --%>
	...
	<script>
		...
		function displayInfowindow(marker, title) {
			...
			infowindow.setContent(content); // 인포 윈도우에 생성될 컨텐트 내용 (클릭시 표시되는 창)
			infowindow.open(map, marker); //  map(표시될 지도)와 marker를 넣고 인포 윈도우 오픈
			
			console.log(title); // 타이틀
			console.log(marker.getPosition()); // 마커의 좌표
			
			var myTitle = title;
			var myPosition = marker.getPosition(); 
			
			// x좌표 y좌표로 나눠줌
			var myX = myPosition.getLat(); 
			var myY = myPosition.getLng();
