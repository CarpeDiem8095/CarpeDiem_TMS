...
-- 후기 조회 게시판 페이징 
 ...
 SELECT ONEDAY_SEQ, NOTE_SEQ, ONEDAY_DELFLAG, ONEDAY_TITLE, 
 		ONEDATE, REPORTCHK, ONE_READCNT, ONEDAY_PUBLIC
	FROM (SELECT ROW_NUMBER () OVER (ORDER BY ONEDATE DESC) RN,
				 ONEDAY_SEQ, NOTE_SEQ, ONEDAY_DELFLAG, ONEDAY_TITLE, 
				 ONEDATE, REPORTCHK, ONE_READCNT, ONEDAY_PUBLIC
					 FROM TRIP.ONEDAY
...
-- 조회수 
	...
	UPDATE TRIP.ONEDAY SET ONEDAY_PUBLIC = 'Y';
...
SELECT * FROM TRIP.REVIEWBOARD r 
WHERE PLACE_SEQ = 97;


SELECT PLACE_NAME , FROM PLACE p WHERE ONEDAY_SEQ = 2;

SELECT * FROM TRIP.COMMENTBOARD c WHERE ONEDAY_SEQ = 95 AND COMM_SEQ =205;
...
DELETE FROM TRIP.COMMENTBOARD
	...
	AND DEPTH >= (SELECT "DEPTH" 
						...
						WHERE COMM_SEQ=227)
	AND REFFER = (SELECT REFFER FROM TRIP.COMMENTBOARD WHERE COMM_SEQ=227);